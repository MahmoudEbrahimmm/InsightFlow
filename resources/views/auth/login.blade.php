...
<form action="{{ url('login') }}" method="POST" class="space-y-4" id="login-form">
    ...
    @error('g-recaptcha-response')
        <span class="text-red-500 text-sm mt-1">{{ $message }}</span>
    @enderror
    <button data-sitekey="{{ config('services.recaptchav3.site_key') }}" data-callback='onSubmit'
        data-action='submit'
        class="g-recaptcha w-full py-3 mt-4 bg-blue-600 rounded-lg font-semibold text-white hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500">Login</button>
